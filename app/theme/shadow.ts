...
function interpolate(i: number, a: number, b: number, a2: number, b2: number) {
  return parseFloat((((i - a) * (b2 - a2)) / (b - a) + a2).toFixed(2))
}
...
function onDepthChange(depth: number) {
  ...
  return {
    height: y,
    opacity: interpolate(depth, 1, 24, 0.2, 0.6),
    radius: interpolate(s.blur, 1, 38, 1, 16), // (s.blur * 0.45).toFixed(2);
    elevation: depth + 1,
  }
}

export const shadow = (depth: number, color: ColorItem = "black") => {
  const { elevation, height, opacity, radius } = onDepthChange(depth)
  return {
